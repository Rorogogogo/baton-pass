...
test_uninstall_both_is_symmetric_and_idempotent() {
  ...
  make_tools "$tools"
  make_agents "$tools"
  printf '{"hooks":{"Stop":[{"matcher":"keep","custom":"group-metadata","hooks":[{"type":"command","command":"\\"%s/bin/baton\\" check"},{"type":"command","command":"keep-me --flag","timeout":5}]}]}}\n' "$REPO" > "$home/.claude/settings.json"
  printf '{"hooks":{"Stop":[{"matcher":"keep","custom":"group-metadata","hooks":[{"type":"command","command":"\\"%s/bin/baton\\" check"},{"type":"command","command":"keep-me --flag","timeout":5}]}]}}\n' "$REPO" > "$home/.codex/hooks.json"
  ...
  grep -Fq '"custom": "group-metadata"' "$home/.claude/settings.json" || fail "Claude Stop group metadata changed"
  grep -Fq '"custom": "group-metadata"' "$home/.codex/hooks.json" || fail "Codex Stop group metadata changed"
  grep -Fq '"timeout": 5' "$home/.claude/settings.json" || fail "unrelated Claude hook metadata changed"
  grep -Fq '"timeout": 5' "$home/.codex/hooks.json" || fail "unrelated Codex hook metadata changed"
